// End-to-end DB test for the Own Your Labs catalog scraper. Sets up OYL as a catalog-mode vendor
// (adapter='ownyourlabs'), links a spread of seed tests, runs discovery against the live site + real
// Postgres, publishes auto-approved changes, and prints the result.
// Run (from apps/worker):  DOTENV_CONFIG_PATH=../../.env npx tsx scripts/discover-ownyourlabs.ts
import 'dotenv/config';
import { prisma } from '@labprice/database';
import { runVendorDiscovery, publishStagedChange } from '../src/discovery';

const SELECTORS = { mode: 'catalog', adapter: 'ownyourlabs', catalogPath: '/shop' };
const SEED_TEST_SLUGS = [
  ...
  'testosterone-total', 'psa-prostate-specific-antigen',
];

async function main() {
  const vendor = await prisma.vendor.upsert({
    where: { slug: 'own-your-labs' },
    update: { websiteUrl: 'https://ownyourlabs.com', isActive: true },
    create: { name: 'Own Your Labs', slug: 'own-your-labs', websiteUrl: 'https://ownyourlabs.com', isActive: true, trustLevel: 'MEDIUM' },
  });
  await prisma.scrapeVendorConfig.upsert({
    where: { vendorId: vendor.id },
    update: { engine: 'HTTP', baseUrl: 'https://ownyourlabs.com', isEnabled: true, selectors: SELECTORS },
    create: { vendorId: vendor.id, engine: 'HTTP', baseUrl: 'https://ownyourlabs.com', isEnabled: true, selectors: SELECTORS },
  });
  ...
  console.log(`Own Your Labs vendor ${vendor.id} · linked ${tests.length}/${SEED_TEST_SLUGS.length} seed tests\n`);
