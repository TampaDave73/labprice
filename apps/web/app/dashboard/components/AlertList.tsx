      ...
      {alerts.map((alert, i) => {
        ...
        return (
          <div
            ...
            style={{ borderTop: i > 0 ? '1px solid oklch(0.94 0.01 230)' : undefined }}
          >
            <div className="flex-1 min-w-0">
              <Link
                href={`/test/${alert.testSlug}`}
                className="text-sm font-semibold text-[oklch(0.2_0.04_230)] no-underline hover:underline"
              >
                {alert.testName}
              </Link>
              <div className="text-xs text-[oklch(0.55_0.04_230)] mt-0.5">{condition}</div>
            </div>
            <span
            ...
            <button
              ...
              className="text-xs text-[oklch(0.5_0.075_230)] hover:text-[oklch(0.4_0.093_230)] cursor-pointer bg-transparent border-none"
            >
              {alert.isActive ? 'Pause' : 'Resume'}
            </button>
            <button
              onClick={() => deleteAlert(alert.id)}
              className="text-xs text-[oklch(0.55_0.04_230)] hover:text-red-600 cursor-pointer bg-transparent border-none"
            >
              Delete
            </button>
          </div>
