...
async function main() {
  const vendor = await prisma.vendor.upsert({
    where: { slug: 'personalabs' },
    update: { websiteUrl: 'https://www.personalabs.com', isActive: true },
    create: { name: 'Personalabs', slug: 'personalabs', websiteUrl: 'https://www.personalabs.com', isActive: true, trustLevel: 'MEDIUM' },
  });
  await prisma.scrapeVendorConfig.upsert({
    where: { vendorId: vendor.id },
    update: { engine: 'HTTP', baseUrl: 'https://www.personalabs.com', isEnabled: true, selectors: SELECTORS },
    create: { vendorId: vendor.id, engine: 'HTTP', baseUrl: 'https://www.personalabs.com', isEnabled: true, selectors: SELECTORS },
  });
