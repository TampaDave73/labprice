...
export async function POST(req: NextRequest) {
  try {
    ...
    // Abuse guard: this endpoint is unauthenticated and fans out to an admin email.
    const rl = rateLimit(getClientIp(req), 'suggest-vendor', { limit: 5, windowMs: 10 * 60 * 1000 });
    if (!rl.ok) {
