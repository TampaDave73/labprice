// End-to-end DB test for the Discounted Labs catalog scraper (adapter='discountedlabs'). Sets up
// Discounted Labs as a catalog-mode vendor, links a spread of seed tests, runs discovery against the
// live site + real Postgres, publishes auto-approved changes, and prints the result.
// Run (from apps/worker):  DOTENV_CONFIG_PATH=../../.env npx tsx scripts/discover-discountedlabs.ts
import 'dotenv/config';
import { prisma } from '@labprice/database';
import { runVendorDiscovery, publishStagedChange } from '../src/discovery';

const SELECTORS = { mode: 'catalog', adapter: 'discountedlabs', catalogPath: '/choose-a-test' };
...
async function main() {
  const vendor = await prisma.vendor.upsert({
    where: { slug: 'discounted-labs' },
    update: { websiteUrl: 'https://www.discountedlabs.com', isActive: true },
    create: { name: 'Discounted Labs', slug: 'discounted-labs', websiteUrl: 'https://www.discountedlabs.com', isActive: true, trustLevel: 'MEDIUM' },
  });
  await prisma.scrapeVendorConfig.upsert({
    where: { vendorId: vendor.id },
    update: { engine: 'HTTP', baseUrl: 'https://www.discountedlabs.com', isEnabled: true, selectors: SELECTORS },
    create: { vendorId: vendor.id, engine: 'HTTP', baseUrl: 'https://www.discountedlabs.com', isEnabled: true, selectors: SELECTORS },
  });
  ...
  console.log(`Discounted Labs vendor ${vendor.id} · linked ${tests.length}/${SEED_TEST_SLUGS.length} seed tests\n`);
