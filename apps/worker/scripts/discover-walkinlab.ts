// End-to-end DB test for the Walk-In Lab catalog scraper (adapter='walkinlab'). Sets up Walk-In Lab
...
// Run (from apps/worker):  DOTENV_CONFIG_PATH=../../.env npx tsx scripts/discover-walkinlab.ts
import 'dotenv/config';
import { prisma } from '@labprice/database';
import { runVendorDiscovery, publishStagedChange } from '../src/discovery';

const SELECTORS = { mode: 'catalog', adapter: 'walkinlab', catalogPath: '/categories/view/all-products' };
...
async function main() {
  const vendor = await prisma.vendor.upsert({
    where: { slug: 'walk-in-lab' },
    update: { websiteUrl: 'https://www.walkinlab.com', isActive: true },
    create: { name: 'Walk-In Lab', slug: 'walk-in-lab', websiteUrl: 'https://www.walkinlab.com', isActive: true, trustLevel: 'MEDIUM' },
  });
  await prisma.scrapeVendorConfig.upsert({
    where: { vendorId: vendor.id },
    update: { engine: 'HTTP', baseUrl: 'https://www.walkinlab.com', isEnabled: true, selectors: SELECTORS },
    create: { vendorId: vendor.id, engine: 'HTTP', baseUrl: 'https://www.walkinlab.com', isEnabled: true, selectors: SELECTORS },
  });
  ...
  console.log(`Walk-In Lab vendor ${vendor.id} · linked ${tests.length}/${SEED_TEST_SLUGS.length} seed tests\n`);
