// End-to-end DB test for the LabCorp OnDemand catalog scraper (adapter='labcorpondemand'). Sets up
// LabCorp OnDemand as a catalog-mode vendor, links a spread of seed tests, runs discovery against the
// live site + real Postgres, publishes auto-approved changes, and prints the result.
// Run (from apps/worker):  DOTENV_CONFIG_PATH=../../.env npx tsx scripts/discover-labcorpondemand.ts
import 'dotenv/config';
import { prisma } from '@labprice/database';
import { runVendorDiscovery, publishStagedChange } from '../src/discovery';

const SELECTORS = { mode: 'catalog', adapter: 'labcorpondemand', catalogPath: '/sitemap.xml' };
...
async function main() {
  const vendor = await prisma.vendor.upsert({
    where: { slug: 'labcorp-ondemand' },
    update: { websiteUrl: 'https://www.ondemand.labcorp.com', isActive: true },
    create: { name: 'LabCorp OnDemand', slug: 'labcorp-ondemand', websiteUrl: 'https://www.ondemand.labcorp.com', isActive: true, trustLevel: 'MEDIUM' },
  });
  await prisma.scrapeVendorConfig.upsert({
    where: { vendorId: vendor.id },
    update: { engine: 'HTTP', baseUrl: 'https://www.ondemand.labcorp.com', isEnabled: true, selectors: SELECTORS },
    create: { vendorId: vendor.id, engine: 'HTTP', baseUrl: 'https://www.ondemand.labcorp.com', isEnabled: true, selectors: SELECTORS },
  });
  ...
  console.log(`LabCorp OnDemand vendor ${vendor.id} · linked ${tests.length}/${SEED_TEST_SLUGS.length} seed tests\n`);
