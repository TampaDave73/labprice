...
- **Internal link graph** — `lib/guides.ts` holds the reverse lookups off `Post.relatedTests`:
  ...
  is no second list to maintain. Rendered by `components/GuideLinks.tsx`.
- **Test copy** (`description`/`purpose`/`procedure`/`preparation`/`normalRange`) is generated by the
  same Claude call as the admin Auto-fill button (`lib/ai/test-lookup.ts`). Bulk path:
  `apps/web/scripts/generate-test-content.ts` writes reviewable JSON to `apps/worker/content/`, then
  `apps/worker/scripts/apply-test-content.ts` writes it to the DB (skips already-populated fields
  unless `--force`, and never touches codes, short names or categories).
...
- **Question research** (`/admin/questions`) — review queue of harvested questions
