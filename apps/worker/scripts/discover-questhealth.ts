// End-to-end DB test for the Quest Health catalog scraper (adapter='questhealth'). Sets up
// Quest Health as a catalog-mode vendor, links a spread of seed tests, runs discovery against the
// live site + real Postgres, publishes auto-approved changes, and prints the result.
// Run (from apps/worker):  DOTENV_CONFIG_PATH=../../.env npx tsx scripts/discover-questhealth.ts
import 'dotenv/config';
import { prisma } from '@labprice/database';
import { runVendorDiscovery, publishStagedChange } from '../src/discovery';

const SELECTORS = { mode: 'catalog', adapter: 'questhealth', catalogPath: '/sitemap_0.xml' };
...
async function main() {
  const vendor = await prisma.vendor.upsert({
    where: { slug: 'quest-health' },
    update: { websiteUrl: 'https://www.questhealth.com', isActive: true },
    create: { name: 'Quest Health', slug: 'quest-health', websiteUrl: 'https://www.questhealth.com', isActive: true, trustLevel: 'MEDIUM' },
  });
  await prisma.scrapeVendorConfig.upsert({
    where: { vendorId: vendor.id },
    update: { engine: 'HTTP', baseUrl: 'https://www.questhealth.com', isEnabled: true, selectors: SELECTORS },
    create: { vendorId: vendor.id, engine: 'HTTP', baseUrl: 'https://www.questhealth.com', isEnabled: true, selectors: SELECTORS },
  });
  ...
  console.log(`Quest Health vendor ${vendor.id} · linked ${tests.length}/${SEED_TEST_SLUGS.length} seed tests\n`);
