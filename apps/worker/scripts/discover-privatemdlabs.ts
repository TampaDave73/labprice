// End-to-end DB test for the Private MD Labs catalog scraper (adapter='privatemdlabs'). Sets up
// Private MD Labs as a catalog-mode vendor, links a spread of seed tests, runs discovery against the
// live site + real Postgres, publishes auto-approved changes, and prints the result.
// Run (from apps/worker):  DOTENV_CONFIG_PATH=../../.env npx tsx scripts/discover-privatemdlabs.ts
import 'dotenv/config';
import { prisma } from '@labprice/database';
import { runVendorDiscovery, publishStagedChange } from '../src/discovery';

const SELECTORS = { mode: 'catalog', adapter: 'privatemdlabs', catalogPath: '/tests?view=all' };
...
async function main() {
  const vendor = await prisma.vendor.upsert({
    where: { slug: 'private-md-labs' },
    update: { websiteUrl: 'https://www.privatemdlabs.com', isActive: true },
    create: { name: 'Private MD Labs', slug: 'private-md-labs', websiteUrl: 'https://www.privatemdlabs.com', isActive: true, trustLevel: 'MEDIUM' },
  });
  await prisma.scrapeVendorConfig.upsert({
    where: { vendorId: vendor.id },
    update: { engine: 'HTTP', baseUrl: 'https://www.privatemdlabs.com', isEnabled: true, selectors: SELECTORS },
    create: { vendorId: vendor.id, engine: 'HTTP', baseUrl: 'https://www.privatemdlabs.com', isEnabled: true, selectors: SELECTORS },
  });
  ...
  console.log(`Private MD Labs vendor ${vendor.id} · linked ${tests.length}/${SEED_TEST_SLUGS.length} seed tests\n`);
